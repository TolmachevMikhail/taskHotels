...
function format_comments($comments) {
    ...
    foreach ($comments as $comment)
        $children[$comment['parent_id']][] = $comment; //par_id :child_comment
    ...
    while ($loop && ( ( $option = each($children[$parent]) ) || ( $parent > $root_id ) )) {
        if ($option === false) {
            ...
            $delete_link = '<a href="#" class="delete_button"  id="%2$s"><img src= "x_mark.png"></a><br/>';
            ...
            $html[] = sprintf(
                ...
                $reply_link .$delete_link. '</li>', str_repeat("\t", ( count($parent_stack) + 1 ) * 2 - 1), // %1$s = tabulation
